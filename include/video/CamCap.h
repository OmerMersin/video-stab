...
namespace vs {
    ...
    public:
        struct Parameters {
            ...
            int queueSize = 5;         ///< Max queue size if using threaded mode (increased for stability)
            int threadTimeout = 500;   ///< Time in ms to wait for a frame in read(); increased for stability
        };
        ...
        bool isHealthy() const; ///< Check if the camera/stream is working properly

        double getFrameRate() const { return framerate; }
        double getWidth() const { return cap.get(cv::CAP_PROP_FRAME_WIDTH); }
        double getHeight() const { return cap.get(cv::CAP_PROP_FRAME_HEIGHT); }
