...
private:
    ...
    // AppSrc elements for feeding data
    ...
    static gboolean busCallback(GstBus* bus, GstMessage* msg, gpointer data) {
        ...
        switch (GST_MESSAGE_TYPE(msg)) {
            ...
            case GST_MESSAGE_STATE_CHANGED: {
                if (GST_MESSAGE_SRC(msg) == GST_OBJECT(manager->outputPipeline) ||
...
public:
    GStreamerPipelineManager(const std::string& address = "rtsp://127.0.0.1:8554/forwarded", 
                           int bitrate = 4000000) 
        : outputAddress(address), bitrate(bitrate), pipelinesInitialized(false),
          outputPipeline(nullptr), passthroughPipeline(nullptr), processingPipeline(nullptr),
          passthroughAppSrc(nullptr), processingAppSrc(nullptr),
          passthroughSink(nullptr), processingSink(nullptr), outputSrc(nullptr),
    ...
    bool initialize(int width, int height, double framerate) {
        ...
        std::string outputPipelineStr = 
            "interpipesrc name=from_input listen-to=passthrough_sink ! "
        ...
        std::string passthroughPipelineStr = 
            ...
            "interpipesink name=passthrough_sink";
        ...
        std::string processingPipelineStr = 
            ...
            ",framerate=" + std::to_string(static_cast<int>(fps)) + "/1 ! "
            "queue max-size-buffers=1 leaky=downstream ! "
            "interpipesink name=processing_sink";
        ...
        if (!passthroughAppSrc || !processingAppSrc || !passthroughSink || !processingSink || !outputSrc) {
        ...
        GstBus* bus = gst_pipeline_get_bus(GST_PIPELINE(outputPipeline));
    ...
    bool switchToPassthrough() {
        ...
        // Switch the output interpipesrc to listen to passthrough_sink
        g_object_set(outputSrc, "listen-to", "passthrough_sink", nullptr);
        
        isCurrentlyPassthrough = true;
        std::cout << "Switched to passthrough mode" << std::endl;
        return true;
    }
    ...
    bool switchToProcessing() {
        ...
        std::cout << "Switched to processing mode" << std::endl;
        return true;
    }
    ...
    bool pushFrame(const cv::Mat& frame) {
        ...
        // Push to both pipelines (they'll handle the data independently)
        GstFlowReturn ret1 = gst_app_src_push_buffer(GST_APP_SRC(passthroughAppSrc), gst_buffer_ref(buffer));
        GstFlowReturn ret2 = gst_app_src_push_buffer(GST_APP_SRC(processingAppSrc), buffer);
        
        return (ret1 == GST_FLOW_OK && ret2 == GST_FLOW_OK);
    }
    ...
    void printStatus() {
        ...
        std::cout << "  - Current Mode: " << (isCurrentlyPassthrough ? "Passthrough" : "Processing") << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::string outputAddress = "rtsp://127.0.0.1:8554/forwarded";  // RTSP output
    int bitrate = std::max(800, std::min(8000, static_cast<int>(frameWidth * frameHeight * fps * 0.1 / 1000)));
    
    GStreamerPipelineManager pipelineManager(outputAddress, bitrate);
    ...
    if (usePassthrough) {
        ...
        std::cout << "Starting in PROCESSING mode" << std::endl;
    }
    ...
    while (!stopRequested) {
        ...
        // Display raw frame occasionally
        ...
        if (!usePassthrough) {
            ...
            try {
                ...
                if (runParams.stabilizationEnabled) {
                    processedFrame = stab.stabilize(processedFrame);
                }
                ...
                std::cerr << "Error in processing pipeline: " << e.what() << std::endl;
                processedFrame = frame.clone();
            }
        }

        // Display final output occasionally
        if (!runParams.optimizeFps && frameCounter % 30 == 0 && !processedFrame.empty()) {
            cv::Mat displayFrame;
            if (windowWidth > 0 && windowHeight > 0) {
        ...
        // Send frame to pipeline manager
        if (!processedFrame.empty()) {
            pipelineManager.pushFrame(processedFrame);
        }
