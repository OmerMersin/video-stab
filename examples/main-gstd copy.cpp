...
public:
    ...
    bool initialize(int width, int height, double framerate) {
        ...
        // Create PASSTHROUGH pipeline (direct H.265 forwarding - no OpenCV processing)
        ...
        // Create PROCESSING pipeline (OpenCV frame processing)
        std::string processingPipelineStr = 
            ...
            "bitrate=" + std::to_string(calculatedBitrate) + " key-int-max=15 intra-refresh=true "
            "b-adapt=0 bframes=0 ref=1 me=dia trellis=0 weightp=0 "
